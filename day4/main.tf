...
resource "aws_instance" "my_linux_vm" {
  ...
  tags = {
    Name = "Linux_${index(keys(aws_subnet.my_subnets),each.key)}_${each.value.availability_zone}"
  }
...
resource "aws_subnet" "my_subnets" {
  ...
  for_each = toset(data.aws_availability_zones.avaliability_zones.names)  
  cidr_block        = cidrsubnet(var.vpc_cidr, 6, index(data.aws_availability_zones.avaliability_zones.names,each.value))
  availability_zone = each.value
  tags = {
    Name = "my_subnet_${each.value}"
  }
